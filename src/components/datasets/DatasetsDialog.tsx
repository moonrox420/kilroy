 ...
 *      Settings → Chat model.
 *
 * Training-environment status sits at the bottom as an informational
 * prerequisite probe. Kilroy does not currently launch LoRA training.
 */
          ...
          <Section title="1 · Load a dataset" icon={<Upload className="h-3 w-3" />}>
            <div className="flex flex-col gap-2">
              ...
              <p className="text-[10.5px] text-ink-subtle">
                ...
                <span className="font-mono">.ndjson</span>. Arrow, Feather,
                and Parquet are not currently supported.
              </p>
              {error && (
...
function TrainingEnvCard({ env }: { env: ReturnType<typeof useDatasets.getState>["trainingEnv"] }) {
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <p className="text-[10.5px] text-ink-subtle">
        Kilroy does not currently launch LoRA training. This panel only
        reports prerequisites; use an external trainer or the Modelfile
        composition above.
      </p>
    </div>
  );
