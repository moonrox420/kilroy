...
import { KilroyWatermark } from "@/components/editor/KilroyWatermark";
import { useWorkspace } from "@/store/workspace";
import { TerminalPanel } from "@/components/terminal/TerminalPanel";
import { AgentHub } from "@/components/chat/AgentHub";
import { useUI } from "@/store/ui";

const MonacoPane = lazy(() => import("@/components/editor/MonacoPane").then((module) => ({ default: module.MonacoPane })));

export function IDELayout() {
  const hasEditor = useWorkspace((state) => state.openTabs.length > 0);
              ...
              >
                ...
                <Panel minSize={8}>
                  <div className="h-full bg-bg-0">
                    {hasEditor ? (
                      <Suspense fallback={<div className="p-4 text-sm text-ink-muted">Loading local editor…</div>}>
                        <MonacoPane />
                      </Suspense>
                    ) : <KilroyWatermark />}
                  </div>
