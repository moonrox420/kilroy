...
export function EditorTabs() {
  ...
  const toggleDiffView = useWorkspace((s) => s.toggleDiffView);
  ...
  return (
    <div className="flex h-8 shrink-0 items-end border-b border-line bg-bg-0">
      ...
      {activeTab?.agentPreview && activeTab.originalContents !== undefined && (
        <div className="mr-2 flex h-5 items-center self-center overflow-hidden rounded border border-amber/40 bg-bg-1 text-[10px]">
          <button
            type="button"
            onClick={() => toggleDiffView(activeTab.path)}
            className={cn(
