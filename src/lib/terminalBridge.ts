...
function getOrCreateBridge(id: string): SessionBridge {
  ...
  if (!bridge) {
    bridge = {
      ...
      unlistenIdleTimeout: null,
      dataListenGen: 0,
      exitListenGen: 0,
      bytesExceededListenGen: 0,
      idleTimeoutListenGen: 0,
      teardownTimer: null,
      attachInFlight: null,
      attachDone: false,
    };
...
function stopIdleTimeoutListener(bridge: SessionBridge) {
  bridge.idleTimeoutListenGen += 1;
  bridge.unlistenIdleTimeout?.();
  bridge.unlistenIdleTimeout = null;
}
...
function ensureIdleTimeoutListener(id: string, bridge: SessionBridge) {
  if (bridge.unlistenIdleTimeout) return;

  const gen = ++bridge.idleTimeoutListenGen;
  void term
    .onIdleTimeout(id, () => {
      if (bridge.idleTimeoutListenGen !== gen) return;
      notify.error(
        "Terminal idle timeout",
        `Session ${id.slice(0, 8)} was stopped after 30 minutes without input or output.`,
      );
    })
    .then((unlisten) => {
      if (bridge.idleTimeoutListenGen !== gen) {
        unlisten();
        return;
      }
      bridge.unlistenIdleTimeout = unlisten;
    })
    .catch((err) => {
      console.error(
        `terminalBridge.listen(idle_timeout)[${id.slice(0, 8)}]:`,
        err,
      );
    });
}
...
function teardownIfIdle(id: string, bridge: SessionBridge) {
  ...
  stopIdleTimeoutListener(bridge);
  bridgeMap().delete(id);
}
...
export function subscribeTerminalSession(
  ...
  ensureIdleTimeoutListener(id, bridge);
