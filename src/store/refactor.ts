 ...
 *
 * Async subscriptions are owned by their mounting component.
 */
...
export const useRefactor = create<RefactorState>((set, get) => ({
  ...
  initListeners() {
    const scope = createListenerScope((error) => notify.fromError("Refactor subscriptions", error));
    ...
    void refactor
      .onScanDone(() => {
        ...
        void refactor.stats().then((stats) => set({ stats })).catch(scope.report);
