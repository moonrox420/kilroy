...
export interface OpenFile {
  ...
  /** When true, renders Monaco DiffEditor side-by-side */
  diffView?: boolean;
  /** Pre-modification contents from disk for diffing */
  originalContents?: string;
  /** Canonical unified diff representation if available */
  diff?: string;
}
...
interface WorkspaceState {
  ...
  toggleDiffView: (path: string) => void;
...
export const useWorkspace = create<WorkspaceState>((set, get) => ({
  ...
  async openFolder(path) {
    ...
    try {
      ...
      set({ rootPath: opened.project.root_path });
  ...
  async showAgentPreview({ actionId, path, contents, diff }) {
    const previewPath = `agent-preview:${actionId}`;
    let originalContents = "";
    try {
      originalContents = await fs.readFile(path);
    } catch {
      // File may not exist on disk yet (e.g. newly created file)
    }
    ...
    const file: OpenFile = {
      ...
      originalContents,
      diff,
      diffView: Boolean(originalContents),
      dirty: false,
      readOnly: true,
      sourcePath: path,
      agentPreview: true,
  ...
  toggleDiffView(path) {
    set((s) => ({
      openTabs: s.openTabs.map((t) =>
        t.path === path ? { ...t, diffView: !t.diffView } : t
      ),
    }));
  },
