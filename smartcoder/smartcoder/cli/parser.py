"""
CLI argument parser and config construction.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        ...
        description="SmartCoder — Kilroy's local, self-correcting coding agent (project-grounded).",
    ...
    parser.add_argument("--max-items", type=int, default=5000, help="Rows per dataset (-1 = all).")
    ...
    sub.add_parser("build-index", help="Build or refresh the FAISS retrieval index, then exit.")

    ask = sub.add_parser("ask", help="Run a single coding task and print the result.")
    ask.add_argument("prompt", nargs="+", help="The coding task (quote it or pass as words).")

    sub.add_parser("chat", help="Start an interactive coding session.")

    ld = sub.add_parser("list-datasets", help="List pre-wired datasets (and optionally the Hub).")
    ld.add_argument("--hub", action="store_true", help="Also query the Hugging Face Hub.")
    ld.add_argument(
        "--filter",
        default=None,
...
def config_from_args(args: argparse.Namespace) -> AppConfig:
    ...
    datasets = tuple(d.strip() for d in args.datasets if d and d.strip()) if args.datasets else ()
