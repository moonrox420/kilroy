...
def _make_llama_cpp_model(config: AppConfig):
    ...
    class LlamaCppModel(Model):
        def __init__(
            ...
            super().__init__(
                model_id=Path(model_path).name, flatten_messages_as_text=True
            )
        ...
        def _normalize(self, messages: list[Any]) -> list[dict[str, str]]:
            ...
            for message in messages:
                ...
                normalized.append(
                    {"role": str(role), "content": self._flatten_content(content)}
                )
            return normalized
        ...
        def generate(
            ...
            **kwargs: Any,
            ...
            try:
                ...
                raise RuntimeError(
                    f"Unexpected model response format: {completion}"
                ) from exc
            return ChatMessage(role="assistant", content=text)  # type: ignore[arg-type]

    return LlamaCppModel(
        model_path=cast(
            str, config.llama_model_path
        ),  # validated non-None in __post_init__
