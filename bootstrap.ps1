...
.DESCRIPTION
    ...
    prerequisites (Rust, Node, MSVC build tools, Ollama, WebView2),
    optionally enables Windows Sandbox, pulls the default chat +
    embedding models, runs `npm install` + `cargo fetch`, then launches
    dev mode. Idempotent — re-run any time.
...
param(
    ...
    [switch]$NoRun,
    [switch]$Build,
    [string]$ChatModel = "qwen2.5-coder:14b-instruct-q8_0",
...
# --- 2/6: toolchain (Rust, Node, MSVC, WebView2) ---------------------------

Write-Phase 2 6 "toolchain (Rust, Node, MSVC, WebView2)"
...
Write-Phase 3 6 "Ollama runtime + models"
...
# --- 4/6: Windows Sandbox feature ------------------------------------------

Write-Phase 4 6 "Windows Sandbox feature"
if ($SkipSandbox) {
...
# --- 5/6: frontend + Rust deps ---------------------------------------------

Write-Phase 5 6 "frontend + Rust deps"
