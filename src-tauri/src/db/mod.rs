...
/// Determine whether `root` is Kilroy's own repo (has tauri.conf.json and package.json).
fn is_kilroy_repo(root: &Path) -> bool {
    root.join("src-tauri").join("tauri.conf.json").is_file() && root.join("package.json").is_file()
}
...
impl Memory {
    ...
    ///   - The folder IS the Kilroy repo (has `src-tauri/tauri.conf.json`), or
    ...
    pub fn open(root: impl AsRef<Path>, app_data_dir: &Path) -> Result<Self> {
        ...
        actions::recover_interrupted(&conn).context("recovering interrupted actions")?;
