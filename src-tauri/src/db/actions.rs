...
/// Atomically reserve a pending action for one executor. This prevents two
/// concurrent Accept requests from performing the same side effect twice.
pub fn claim_pending(conn: &Connection, id: i64) -> Result<bool> {
    Ok(conn.execute(
        "UPDATE actions SET status = 'accepted' WHERE id = ?1 AND status = 'pending'",
        params![id],
    )? == 1)
}

/// Resolve only an action that is still pending.
pub fn reject_pending(conn: &Connection, id: i64) -> Result<bool> {
    Ok(conn.execute(
        "UPDATE actions
         SET status = 'rejected', error = NULL, resolved_at = strftime('%s', 'now')
         WHERE id = ?1 AND status = 'pending'",
        params![id],
    )? == 1)
}

/// An accepted action left behind by a terminated process has an unknown
/// outcome and must never be executed automatically on restart.
pub fn recover_interrupted(conn: &Connection) -> Result<usize> {
    Ok(conn.execute(
        "UPDATE actions
         SET status = 'failed',
             error = COALESCE(error, 'Application stopped while applying this action; inspect the target before retrying.'),
...
mod tests {
    ...
    fn pending_action(conn: &Connection) -> i64 {
        insert(
            conn,
            NewAction {
                session_id: None,
                task_id: None,
                kind: "shell",
                target: Some("echo test"),
                payload: r#"{"kind":"shell","command":"echo test","sandbox":"host"}"#,
                diff: None,
            },
        )
        .unwrap()
    ...
    #[test]
    fn pending_action_can_only_be_claimed_once() {
        let conn = database();
        let id = pending_action(&conn);

        assert!(claim_pending(&conn, id).unwrap());
        assert!(!claim_pending(&conn, id).unwrap());
        assert!(!reject_pending(&conn, id).unwrap());
        assert_eq!(fetch(&conn, id).unwrap().status, "accepted");
    }

    #[test]
    fn interrupted_claim_is_failed_without_reexecution() {
        let conn = database();
        let id = pending_action(&conn);
        assert!(claim_pending(&conn, id).unwrap());

        assert_eq!(recover_interrupted(&conn).unwrap(), 1);
        let action = fetch(&conn, id).unwrap();
        assert_eq!(action.status, "failed");
        assert!(action.error.unwrap().contains("inspect the target"));
    }
}
