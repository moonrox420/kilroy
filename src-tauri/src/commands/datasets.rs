...
//!
//!   1. **Inspect** — open a `.json` or `.jsonl` file, detect
...
//!
//!   3. **LoRA prerequisite probe** — reports whether Python, supported
//!      training libraries, and an NVIDIA GPU are visible. Kilroy does not
//!      currently launch a fine-tuning process.
//!
    ...
    match ext.as_str() {
        ...
        "arrow" | "feather" | "parquet" => Err(format!(
            "{} files need Python + pyarrow to inspect. Install with: \
             `pip install pyarrow`, but inspection for this format is not \
             implemented in Kilroy.",
            ext
        )),
        other => Err(format!(
            "unsupported dataset extension `.{}` — accepted: .json, .jsonl, .ndjson",
            other
        )),
    }
}
...
/// Probe for a possible external training stack. This command does not launch
/// training; it only reports locally observable prerequisites.
#[tauri::command]
