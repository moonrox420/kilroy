...
/// Resolve skill directories without writing into projects during reads.
fn dirs(app: &AppHandle, project_root: Option<&Path>) -> Result<(PathBuf, Option<PathBuf>)> {
    ...
    let project = project_root
        .map(|root| crate::actuator::resolve_safe(root, ".kilroy/skills"))
        .transpose()?;
    Ok((global, project))
}
...
pub async fn read_skill(
    ...
    for c in candidates {
        if c.is_file() {
            let root = c.parent().ok_or("skill has no parent directory")?;
            let safe = crate::actuator::resolve_safe(root, &format!("{name}.md"))
            ...
            if std::fs::metadata(&safe)
                ...
                > 256 * 1024
            {
                return Err("Skill exceeds the 256 KiB read limit".into());
            }
            return std::fs::read_to_string(&safe)
                .map_err(|e| format!("read {}: {}", safe.display(), e));
        }
    }
...
pub async fn write_skill(
    ...
    if content.len() > 256 * 1024 {
        return Err("Skill exceeds the 256 KiB size limit".into());
    }
    let project_root = state.memory.lock().as_ref().map(|m| m.root.clone());
    let (global_dir, project_dir) =
        dirs(&app, project_root.as_deref()).map_err(|e| format!("{e:#}"))?;
    let target_dir = match scope.as_str() {
        "global" => global_dir,
        "project" => project_dir.ok_or("no project open")?,
        _ => return Err("scope must be global or project".into()),
    };
    std::fs::create_dir_all(&target_dir)
        .map_err(|error| format!("create skills directory: {error}"))?;
    let target = crate::actuator::resolve_safe(&target_dir, &format!("{name}.md"))
        .map_err(|error| error.to_string())?;
    crate::actuator::apply_file_write(&target_dir, &format!("{name}.md"), &content)
        .map_err(|error| error.to_string())?;
    tracing::info!(skill = %name, scope = %scope, "wrote skill");
    Ok(target.to_string_lossy().to_string())
}
...
mod tests {
    ...
    #[test]
    fn skill_names_cannot_escape_the_skill_directory() {
        for invalid in [
            "../secrets",
            "..\\secrets",
            "C:secret",
        ...
        assert!(validate_skill_name("python-style_3").is_ok());
    }
