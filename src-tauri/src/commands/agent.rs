...
//! Behaviour by mode:
//!
//! * CodeAgent — project-grounded, approval-gated Rust tool runtime.
...
use super::agent_context::{gather_agent_context, ActiveEditorFile, BuiltAgentContext};
...
pub async fn agent_send_message(
    ...
    let built = gather_agent_context(&app, &state, &user_msg, payload.active_file).await;
    ...
    match mode {
        AgentMode::CodeAgent => {
            let (content, returned_run_id) = if built.project_root.is_none()
                || is_conversational_query(&user_msg)
            {
                (
                    run_single_shot(
                        &app,
                ...
                let request = AgentRequest {
                    ...
                    mode: RuntimeMode::Code,
                    message: user_msg.clone(),
                    images: payload.images.clone(),
                };
                match crate::runtime::coder::run_editor_coder(&app, state.inner(), request, &built)
                    .await
                {
                    Ok(run) => (run.summary, Some(run.run_id)),
                    Err(error) => (format!("Editor coder failed: {error:#}"), None),
                }
            };
...
// ─── Code Agent (native, project-grounded) ────────────────────────────────────

struct ToolAgentInput<'a> {
    user_msg: &'a str,
    built: &'a BuiltAgentContext,
    run_id: &'a str,
    session_id: Option<i64>,
    mode: RuntimeMode,
...
async fn run_tool_agent(
    app: &AppHandle,
    state: &State<'_, AppState>,
    input: ToolAgentInput<'_>,
) -> String {
    let request = AgentRequest {
        run_id: input.run_id.to_string(),
        session_id: input.session_id,
        mode: input.mode,
        message: input.user_msg.to_string(),
        images: input.images,
    };
    match rust_agent::run_code(app, state.inner(), request, input.built).await {
        Ok(run) => run.summary,
