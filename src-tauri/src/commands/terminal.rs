...
//!   write from the frontend). A watchdog checks every 30 seconds; if the
//!   gap since `pipe.last_write_ts` exceeds the timeout, the child is killed
//!   and the UI is notified via `terminal://{id}/idle_timeout`.
//!
//! Both limits terminate the actual child process rather than merely stopping
//! event delivery.
//!
...
use crate::state::{AppState, PtySession, TerminalPipe, PTY_IDLE_TIMEOUT};
...
pub async fn terminal_spawn(
    ...
    let reader_sessions = state.ptys.clone();
            ...
            match reader.read(&mut buf) {
                ...
                Ok(n) => {
                    ...
                    if p.output_bytes.saturating_add(n) > PTY_BYTE_CAP {
                        drop(p);
                        if let Some(session) = reader_sessions.lock().get_mut(&reader_session_id) {
    ...
    let idle_sessions = state.ptys.clone();
    let idle_session_id = id.clone();
    let idle_app = app.clone();
    let idle_event = format!("terminal://{}/idle_timeout", id);
    std::thread::spawn(move || loop {
        std::thread::sleep(std::time::Duration::from_secs(30));
        let mut sessions = idle_sessions.lock();
        let Some(session) = sessions.get_mut(&idle_session_id) else {
            return;
        };
        let mut session_pipe = session.pipe.lock();
        if session_pipe.exited || session_pipe.last_write_ts.is_none() {
            return;
        }
        let expired = idle_timeout_reached(session_pipe.last_write_ts);
        if !expired {
            continue;
        }
        session_pipe.last_write_ts = None;
        drop(session_pipe);
        let _ = session.child.kill();
        drop(sessions);
        let _ = idle_app.emit(&idle_event, ());
        return;
    });
...
fn idle_timeout_reached(last_activity: Option<std::time::Instant>) -> bool {
    last_activity.is_some_and(|last| last.elapsed() >= PTY_IDLE_TIMEOUT)
}
...
mod tests {
    ...
    fn idle_timeout_requires_a_full_inactive_interval() {
        assert!(!idle_timeout_reached(None));
        assert!(!idle_timeout_reached(Some(std::time::Instant::now())));
        assert!(idle_timeout_reached(Some(
            std::time::Instant::now() - PTY_IDLE_TIMEOUT - std::time::Duration::from_secs(1)
        )));
    }
}
