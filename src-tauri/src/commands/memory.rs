...
use crate::db::{
    activity, chunks, decisions, embedding_profile, files, messages, projects, sessions, tasks,
    Memory,
};
...
pub async fn open_project(
    ...
    let _project_access = state.project_access.try_write().map_err(|_| {
        "A project operation is still running. Wait for it to finish before switching folders."
            .to_string()
    })?;
    ...
    let memory = Memory::open(&p, &app_data_dir).map_err(|e| format!("open memory: {:#}", e))?;
...
async fn check_ollama(state: &State<'_, AppState>) -> OllamaStatus {
    ...
    match emb.health().await {
        Ok(models) => {
            let has = models
                .iter()
                .any(|m| super::settings::matches_model(m, &embedding_model));
            OllamaStatus {
                reachable: true,
                models,
                embedding_model,
...
pub async fn index_project(
    ...
    let (window, stride) = {
        ...
        (settings.chunk_window, settings.chunk_stride)
    };
    embedder
        .embed("Kilroy index readiness probe")
        .await
        .map_err(stringify)?;
    embedding_profile::prepare_index(&memory_conn.lock(), &profile).map_err(stringify)?;
    ...
    for (i, path) in candidates.iter().enumerate() {
        ...
        let hash = files::hash(&format!("{profile}\n{window}:{stride}\n{text}"));
        let language = language_for_path(path);

        // Read-only deduplication; failed embeddings must not mark the file indexed.
        let needs_reindex = {
            let conn = memory_conn.lock();
            match files::needs_index(&conn, project_id, &rel.to_string_lossy(), &hash) {
                Ok(t) => t,
                Err(e) => {
                    tracing::warn!(file = %rel.display(), "index lookup: {:#}", e);
                    result.errors += 1;
                    continue;
                }
            }
        ...
        if let Err(e) = (|| -> anyhow::Result<()> {
            embedding_profile::ensure(&conn, &profile)?;
            files::replace_index(
                &conn,
                files::IndexedFile {
                    project_id,
    ...
    // Re-embed preserved decision text after a model change, retrying missing vectors.
    let pending_decisions = {
        let conn = memory_conn.lock();
        let mut statement = conn.prepare("SELECT id, title, summary, COALESCE(rationale, '') FROM decisions WHERE project_id = ?1 AND id NOT IN (SELECT decision_id FROM decision_embeddings)").map_err(stringify)?;
        let rows = statement
    ...
    for (id, text) in pending_decisions {
        match embedder.embed(&text).await {
            Ok(vector) => {
                let conn = memory_conn.lock();
                embedding_profile::ensure(&conn, &profile).map_err(stringify)?;
                decisions::insert_embedding(&conn, id, &vector).map_err(stringify)?;
            }
            Err(error) => {
                tracing::warn!(decision_id = id, "decision embedding failed: {error:#}");
                result.errors += 1;
            }
        }
    }
...
pub async fn search_memory(
    ...
    let (chunks_hits, decision_hits) = {
        let conn = memory_conn.lock();
        embedding_profile::ensure(&conn, &embedder.profile()).map_err(stringify)?;
...
pub async fn log_decision(
    ...
    let conn = memory_conn.lock();
    embedding_profile::ensure(&conn, &embedder.profile()).map_err(stringify)?;
    let transaction = conn.unchecked_transaction().map_err(stringify)?;
    ...
    .map_err(stringify)?;
    decisions::insert_embedding(&transaction, id, &vec).map_err(stringify)?;
