...
///
/// * `CodeAgent` — native read-only investigation with approval-gated mutations.
...
/// * `Debug` — the durable evidence-driven runtime in review/debug mode; it
///   inspects the project with read-only tools and gates proposed changes.
...
pub struct AppState {
    /// Keep a project's DB, root, and session stable throughout async operations.
    pub project_access: tauri::async_runtime::RwLock<()>,
...
impl AppState {
    pub fn new(settings: Settings) -> Self {
        ...
        Self {
            project_access: tauri::async_runtime::RwLock::new(()),
    ...
    /// Kill owned terminal processes and the bundled Ollama daemon at app
    /// exit. Wired from lib.rs as a RunEvent::Exit handler.
    pub fn shutdown(&self) {
        let mut sessions = self.ptys.lock();
        for (id, mut session) in sessions.drain() {
            tracing::debug!(terminal_id = %id, "killing terminal child on app exit");
