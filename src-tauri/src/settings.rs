...
impl Settings {
    pub fn validate(&self) -> Result<()> {
        let url = reqwest::Url::parse(&self.ollama_url).context("invalid Ollama URL")?;
        ...
        {
            bail!("Ollama URL must be HTTP(S), with no credentials, query, or fragment");
        }
        if self.chat_model.trim().is_empty() || self.embedding_model.trim().is_empty() {
            bail!("chat and embedding model names cannot be empty");
        }
        if self.embedding_dim != crate::db::schema::EMBEDDING_DIM {
            bail!("embedding dimension must match the 768-dimensional database schema");
        }
        if self.chunk_stride > self.chunk_window {
            bail!(
                "chunk stride cannot exceed chunk window: that would leave source lines unindexed"
            );
        }
        Ok(())
    }
        ...
        match std::fs::read_to_string(&path) {
            Ok(raw) => match serde_json::from_str::<Self>(&raw) {
                Ok(s) => match s.validate() {
                    Ok(()) => (s, true),
                    Err(error) => {
                        tracing::error!("Invalid settings preserved on disk: {error:#}");
                        (Self::default(), true)
                    }
                },
                Err(e) => {
                    tracing::error!(
                        "settings.json parse failed ({e}); preserving the file for recovery"
                    );
                    (Self::default(), true)
                }
            },
    ...
    pub fn save(&self, app: &AppHandle) -> Result<()> {
        ...
        if result.is_err() && temporary.exists() {
            if let Err(error) = std::fs::remove_file(&temporary) {
                tracing::warn!("remove failed settings temporary file: {error}");
            }
        }
        result?;
        Ok(())
...
impl SettingsUpdate {
    pub fn apply(self, s: &mut Settings) {
        if let Some(v) = self.ollama_url {
            s.ollama_url = v.trim().trim_end_matches('/').to_string();
        }
        if let Some(v) = self.chat_model {
            s.chat_model = v.trim().to_string();
        }
...
mod tests {
    ...
    #[test]
    fn missing_persisted_fields_receive_current_defaults() {
        let settings: Settings =
            serde_json::from_str(r#"{"ollama_url":"http://127.0.0.1:11434","first_run":false}"#)
                .unwrap();
    ...
    #[test]
    fn rejects_configuration_that_loses_source_lines_or_exposes_credentials() {
        let mut settings = Settings::default();
        settings.chunk_stride = settings.chunk_window + 1;
