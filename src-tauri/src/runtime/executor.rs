...
use crate::db::{
    actions as db_actions, agent_runtime, chunks::ChunkHit, decisions::DecisionHit, tasks,
};
...
fn extract_and_persist_actions(
    ...
    let mut blocks = parser::extract_blocks(output);

    // If output contains raw SEARCH/REPLACE blocks outside of code fences, extract them
    if output.contains("<<<<<<< SEARCH") && output.contains(">>>>>>> REPLACE") {
        let sr_blocks = actuator::search_replace::extract_search_replace_blocks(output);
        for sr in sr_blocks {
            let already_captured = blocks.iter().any(|b| b.body.contains(&sr.search));
            if !already_captured {
                if let Some(path) = sr.path {
                    let body = format!(
                        "<<<<<<< SEARCH\n{}\n=======\n{}\n>>>>>>> REPLACE",
                        sr.search, sr.replace
                    );
    ...
    for block in &blocks {
        ...
        let (kind, target, diff, preview) = match &payload {
            ...
            ActionPayload::FilePatch { path, unified_diff } => {
                ...
                let effective_diff = if let Some(preview_content) = &preview {
                    actuator::diff_against_disk(&ctx.project_root, path, preview_content)
            ...
            ActionPayload::FileChange { .. } => continue,
        };
