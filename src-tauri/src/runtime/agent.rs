...
pub async fn run_code(
    ...
    let shell_instruction = match default_sandbox {
        SandboxKind::Docker => "Shell proposals run as POSIX sh inside Docker at /work. Use POSIX shell syntax, not PowerShell. Dependencies must already be in the image; networking is disabled.",
        SandboxKind::WindowsSandbox => "Shell proposals run as Windows PowerShell 5.1 inside Windows Sandbox. Use PowerShell syntax, not Bash or cmd.exe syntax. Networking is disabled.",
        SandboxKind::Host if cfg!(windows) => "Shell proposals run as Windows PowerShell on the host with no OS isolation. Use PowerShell syntax.",
        SandboxKind::Host => "Shell proposals run as POSIX sh on the host with no OS isolation.",
    };
    evidence.push_str(&format!("\n{shell_instruction}\nCommands use a disposable source copy. Generated file changes require separate approval.\n"));
...
pub(crate) fn persist_proposal(
    ...
    default_sandbox: &SandboxKind,
) -> Result<i64> {
    if let ActionPayload::Shell { sandbox, .. } = &mut payload {
        *sandbox = default_sandbox.clone();
    }
    let policy_input = match &payload {
        ...
        ActionPayload::FileChange { .. } => {
            return Err(anyhow!(
                "file_change is reserved for application-captured sandbox output"
            ))
        }
    };
    crate::runtime::policy::evaluate_policy(policy_input)?;
    let (kind, target, diff, preview) = match &payload {
        ...
        ActionPayload::FileChange { .. } => unreachable!("reserved payload rejected above"),
    };
...
pub(crate) fn emit_progress(app: &AppHandle, run_id: &str, step: usize, kind: &str, message: &str) {
    let _ = app.emit(
        "agent://runtime/event",
        AgentEvent {
            run_id: run_id.to_string(),
