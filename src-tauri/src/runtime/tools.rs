...
fn read_file(
    ...
    if actuator::is_protected_path(path) {
        return Err(anyhow!("protected internal path: {path}"));
    }
    ...
    if lines.is_empty() && start == 1 {
        return Ok(format!("FILE {path}: empty file"));
    }
...
fn search_files(root: &Path, query: &str, relative: Option<&str>) -> Result<String> {
    ...
    let root = root.canonicalize().context("canonicalize search root")?;
    let mut visited = 0usize;
    let mut scanned = 0usize;
    let mut matches = Vec::new();
    while let Some(path) = stack.pop() {
        if visited >= MAX_SEARCH_FILES || matches.len() >= MAX_SEARCH_MATCHES {
            break;
        }
        visited += 1;
